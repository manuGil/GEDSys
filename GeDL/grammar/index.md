...
```
Event    ::= 'event' EventName '(' EventParameter ( ',' EventParameter )* ')' '{' DetectionRule '}' ';'
```
...
![DetectionRule](diagram/DetectionRule.svg)
...
```
Function ::= FunctionName '(' ( EventName | Function ) ')'
```
...
```
DetectionExtent
         ::= 'extent' Identifier '=' '{' 'feature' ':' FeatureList ',' 'srs' ':' EPSGCode ( ',' 'buffer' ':' Distance )? '};'
```
